...
const addFilterToIncludeObject = ({AND, includeObject}) => {
    ...
    for(const value of AND){
        for(const keys of Object.keys(value)){
            ...
            const valueToAdd = {
                [keyValues.at(-1)]: !Object.values(value)[0].includes(specialSymbol)? {equals: Object.values(value)[0]} : {contains: Object.values(value)[0].replace('$', '')}
            }
            keyValues.pop()
...
class QueryBuilder{
    buildQuery({skip, take, order, sort, AND, includeObject}){
        ...
        const {defaultObject: defaultFilter, includeObject: mutatedInclude} =  addFilterToIncludeObject({AND, includeObject: include})
